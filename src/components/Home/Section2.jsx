...
const Section2 = () => {
    ...
    const servicesData = companyData?.servicesSection;
    const faqSection = companyData?.faqSection;
    ...
    return (
        <section
            className={`font-[Be_Vietnam_Pro] px-4 sm:px-8 py-14 space-y-20 ${companyData?.theme?.bg}`}
        >
            {/* Who We Are Section - Grid Layout */}
            <div
                className={`${companyData?.theme?.bg} backdrop-blur-lg border border-gray-700 rounded-3xl shadow-xl p-8 md:p-12`}
                data-aos="fade-up"
            >
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
                    ...
                    <div className="space-y-6 order-2">
                        ...
                        <p
                            className={`${companyData?.theme?.text} text-base md:text-lg leading-relaxed`}
                        >
                            <span
                                className={`${companyData?.theme?.gradientText} font-semibold`}
                            >
                                {companyData?.name}
                            </span>{" "}
            ...
            <div
                className={`${companyData?.theme?.bg} border border-gray-600 rounded-3xl shadow-xl p-8 md:p-12`}
                data-aos="fade-up"
            >
                <h1 className="text-2xl md:text-3xl font-bold text-center mb-10 text-gray-500">
                    Why{" "}
                    <span className={`${companyData?.theme?.gradientText}`}>
                        Choose Us?
                    </span>
                </h1>
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {companyData?.whychoose?.map((feature, index) => (
                        <div
                            key={index}
                        ...
                        >
                            ...
                            <h2
                                className={`text-lg font-semibold ${companyData?.theme?.primary} mb-2`}
                            >
                                {feature.title}
                            </h2>
                            <p
                                className={`text-sm ${companyData?.theme?.text}`}
                            >
                                {feature.description}
                            </p>
                        </div>
            ...
            <div className="space-y-10" data-aos="fade-up">
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {servicesData?.services?.map((service, index) => (
                        <div
                            key={index}
                            className={`${companyData?.theme?.bg} border rounded-3xl p-8 shadow-lg`}
                            data-aos="fade-up"
                            data-aos-delay={index * 150}
                        >
                            <h2
                                className={`text-xl font-bold ${companyData?.theme?.primary} mb-4`}
                            >
                                {service.title}
                            </h2>

                            <p className={`${companyData?.theme?.text} mb-4`}>
                                {service.description}
                            </p>
            ...
            {/* Our Process Section */}
            <div
                className={`${companyData?.theme?.bg} border border-gray-700 rounded-3xl shadow-xl p-8 md:p-12`}
                data-aos="fade-up"
            >
                <h1 className="text-4xl md:text-3xl font-bold text-center mb-12 text-gray-500">
                  
                    <span className={`${companyData?.theme?.gradientText}`}>
                        {companyData?.faqSection?.heading}
                    </span>
                </h1>
                <div className="space-y-8">
                    {faqSection?.items?.map((item, index) => (
                        ...
                        >
                            <div
                                className={`text-xl font-bold ${companyData?.theme?.gradientText} opacity-50`}
                            >
                                {item.step}
                            </div>
                            <div className="flex-1">
                                <h3
                                    className={`text-lg md:text-xl font-bold ${companyData?.theme?.primary} mb-2`}
                                >
                                    {item.title}
                                </h3>
                                <p
                                    className={`${companyData?.theme?.text} text-base leading-relaxed`}
                                >
                                    {item.description}
                                </p>
                            </div>
            ...
            {/* Experience Section */}
            <div
                className={`relative w-full py-20 ${companyData?.theme?.bg} border border-gray-700 ${companyData?.theme?.text} rounded-3xl overflow-hidden`}
                data-aos="fade-up"
            >
                <div className="relative z-10 max-w-6xl mx-auto px-6 space-y-6">
                    <h1 className="text-2xl md:text-5xl font-bold">
                        Experience {companyData?.name}
                    </h1>
